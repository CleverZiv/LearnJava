...
/**
    ...
    public V get() throws InterruptedException, ExecutionException {
        //获取当前任务状态
        int s = state;
        //条件成立：未执行、正在执行、正在完成，调用get的外部线程会被阻塞在此
        if (s <= COMPLETING)
            //如何实现阻塞？整个类的核心在此
            //返回task当前状态，可能当前线程在里面已经睡了一会儿了
        ...
        for (WaitNode q; (q = waiters) != null;) {
            if (WAITERS.weakCompareAndSet(this, q, null)) {
                for (;;) {
                    ...
                    if (t != null) {
                        q.thread = null;
                        //唤醒当前node节点的线程，挂起的时间是在awaitDone()方法里
    ...
    private int awaitDone(boolean timed, long nanos)
        ...
        for (;;) {
            ...
            if (s > COMPLETING) {
                ...
                Thread.yield();
            //条件成立，说明当前线程唤醒 是被其它线程使用中断这种方式唤醒的 interrupted()返回true之后
            //会将thread的中断标记重置回false
            ...
            //条件成立，第一次自旋，当前线程还未创建 waitNode对象，此时为当前线程创建waitNode对象
            else if (q == null) {
                if (timed && nanos <= 0L)
                    return s;
                q = new WaitNode();
            }
            //条件成立，第二次自旋，当前线程已经创建 waitNode对象，但当前线程还未入队
            else if (!queued) {
                q.next = waiters;
                //cas方式设置waiters引用指向当前线程node，成功的话，queued为true
                queued = WAITERS.weakCompareAndSet(this, waiters, q);
            }
            //条件成立，第三次自旋，timed默认为false
            else if (timed) {
                ...
                //当前调用get操作的线程就会被park，线程状态会变味waiting状态，相当于休眠了....
                //除非有其他线程将你唤醒，或者将当前线程 中断
